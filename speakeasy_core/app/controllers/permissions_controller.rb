class PermissionsController < ApplicationController
  ...
  def create
    ...
    if permission.save
      ...
      render json: {
        message: "User already has permission to this room!" },
        status: :bad_request
    end
  end
  ...
  def invitee
    ...
    unless @invitee
      render json: {
        message: "Hmm, we couldn't find that user in the system."},
        status: :bad_request
      false
    end
  end
  ...
  def confirm_room_access
    head status: :unauthorized unless @room.permissions.pluck(
      :sid).include?(@user.sid)
  end
